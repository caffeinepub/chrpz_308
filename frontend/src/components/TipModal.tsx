...
import { Loader2, DollarSign, Wallet, Info } from 'lucide-react';
import { useTipPost, useGetCallerWallet, useGetCallerImportedTokens } from '../hooks/useQueries';
...
export default function TipModal({ isOpen, onClose, postId }: TipModalProps) {
  ...
  const balanceDisplay = Number(balance) / Math.pow(10, decimals);
  ...
  const handleSendTip = async () => {
    ...
    const tipAmountSmallestUnit = BigInt(Math.floor(tipAmount * Math.pow(10, decimals)));
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md glass-dark border-2">
        <DialogHeader>
          ...
          <DialogDescription>Support the creator with crypto tokens</DialogDescription>
        </DialogHeader>

        <div className="space-y-4">
          {/* Token Selection */}
          <div className="space-y-2">
            <Label htmlFor="token" className="text-foreground">Select Token</Label>
                    ...
                    <div className="flex items-center justify-between w-full gap-4">
                      ...
                      {token.value === 'ICP' && (
                        <span className="text-xs text-muted-foreground">
                          {(Number(token.balance) / Math.pow(10, token.decimals)).toFixed(token.decimals)}
                        </span>
                      )}
                    </div>
                  </SelectItem>
          ...
          {selectedToken === 'ICP' && (
            <Alert className="glass-dark border-2 border-primary/50">
              ...
              <AlertDescription>
                <strong>Your Balance:</strong> {balanceDisplay.toFixed(decimals)} {tokenDisplayName}
              </AlertDescription>
            </Alert>
          )}
          ...
          <Alert className="glass-dark border-2 border-blue-500/50">
            ...
            <AlertDescription className="text-xs">
              <strong>Tipping with Tokens:</strong> You can tip with ICP or any imported token. 
              The recipient will receive the token in their wallet.
            </AlertDescription>
          </Alert>

          <div className="space-y-2">
            <Label htmlFor="amount" className="text-foreground">Amount ({tokenDisplayName === 'ICP' ? 'ICP' : 'tokens'})</Label>
            <Input
              id="amount"
              type="number"
              step="0.01"
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={onClose} variant="outline" disabled={tipPost.isPending}>
              Cancel
            </Button>
          </div>
        </div>
